            ...
            <div className="full_grid">
                <div className="row_1">
                    <div>
                      ...
                      {this.state.nameChosen && (this.state.name === this.state.nameOfPlayerWhoseTurnItIs) ? <button onClick={() => PubSub.publish('pass-turn-button', this.state.name)}>Pass turn</button> : null}
                    </div>
                </div>
                <div className="row_2">
                    <div>
                ...
                <div className="row_4">
                    <div>
                      ...
                      in the lower field (maybe because of separating out visible and inGame?), but it works as is.*/}
                        {this.state.nameChosen ? <button onClick={() => PubSub.publish('join-leave-button')}>Leave Game</button> : <button onClick={() => PubSub.publish('join-leave-button')}>Enter Game</button>}
